...
from mitiq.calibration import Calibrator, Settings, ZNESettings, execute_with_mitigation
from mitiq.calibration.calibrator import convert_to_expval_executor, ExperimentResults, MissingResultsError
from mitiq.calibration.settings import Strategy, MitigationTechnique, BenchmarkProblem
...
def test_best_strategy():
    test_strategy_settings = Settings(benchmarks=[{'circuit_type': 'ghz', 'num_qubits': 2, 'circuit_depth': 10}, {'circuit_type': 'mirror', 'num_qubits': 2, 'circuit_depth': 10, 'circuit_seed': 1}], strategies=[{'technique': 'zne', 'scale_noise': fold_global, 'factory': RichardsonFactory([1.0, 2.0, 3.0])}, {'technique': 'zne', 'scale_noise': fold_global, 'factory': RichardsonFactory([1.0, 3.0, 5.0])}, {'technique': 'zne', 'scale_noise': fold_global, 'factory': LinearFactory([1.0, 2.0, 3.0])}, {'technique': 'zne', 'scale_noise': fold_global, 'factory': LinearFactory([1.0, 3.0, 5.0])}])
...
def test_convert_to_expval_executor():
    ...
    noiseless_expval_executor = convert_to_expval_executor(noiseless_bitstring_executor, bitstring='00')
...
def test_execute_with_mitigation(monkeypatch):
    cal = Calibrator(execute, ZNESettings)
    expval_executor = convert_to_expval_executor(Executor(execute), bitstring='00')
    rb_circuit = generate_rb_circuits(2, 10)[0]
    rb_circuit.append(cirq.measure(rb_circuit.all_qubits()))
...
def test_ExtrapolationResults_add_result():
    ...
    er.add_result(strat, problem, ideal_val=1.0, noisy_val=0.8, mitigated_val=0.9)
    assert er.is_missing_data()
