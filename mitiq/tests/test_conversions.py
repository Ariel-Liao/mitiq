...
import qiskit
from braket.circuits import Circuit as BKCircuit, gates as braket_gates, Instruction
import pennylane as qml
from mitiq import SUPPORTED_PROGRAM_TYPES
from mitiq.interface import convert_to_mitiq, convert_from_mitiq, accept_any_qprogram_as_input, atomic_one_to_many_converter, noise_scaling_converter, UnsupportedCircuitError
...
braket_circuit = BKCircuit([Instruction(braket_gates.H(), 0), Instruction(braket_gates.CNot(), [0, 1])])
circuit_types = {'cirq': cirq.Circuit, 'qiskit': qiskit.QuantumCircuit, 'pyquil': Program, 'braket': BKCircuit, 'pennylane': qml.tape.QuantumTape}
...
def test_to_mitiq_bad_types(item):
    with pytest.raises(UnsupportedCircuitError, match='Could not determine the package of the input circuit.'):
