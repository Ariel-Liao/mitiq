import pytest
import numpy as np
from braket.circuits import Circuit as BKCircuit, Instruction, gates as braket_gates
...
def test_from_braket_non_parameterized_single_qubit_gates():
    ...
    instructions = [Instruction(braket_gates.I(), target=0), Instruction(braket_gates.X(), target=1), Instruction(braket_gates.Y(), target=2), Instruction(braket_gates.Z(), target=3), Instruction(braket_gates.H(), target=0), Instruction(braket_gates.S(), target=1), Instruction(braket_gates.Si(), target=2), Instruction(braket_gates.T(), target=3), Instruction(braket_gates.Ti(), target=0), Instruction(braket_gates.V(), target=1), Instruction(braket_gates.Vi(), target=2)]
...
def test_from_braket_non_parameterized_two_qubit_gates():
    braket_circuit = BKCircuit()
    instructions = [Instruction(braket_gates.CNot(), target=[2, 3]), Instruction(braket_gates.Swap(), target=[3, 4]), Instruction(braket_gates.ISwap(), target=[2, 3]), Instruction(braket_gates.CZ(), target=(3, 4)), Instruction(braket_gates.CY(), target=(2, 3))]
    for instr in instructions:
        braket_circuit.add_instruction(instr)
    cirq_circuit = from_braket(braket_circuit)
    qreg = LineQubit.range(2, 5)
    expected_cirq_circuit = Circuit(ops.CNOT(*qreg[:2]), ops.SWAP(*qreg[1:]), ops.ISWAP(*qreg[:2]), ops.CZ(*qreg[1:]), ops.ControlledGate(ops.Y).on(*qreg[:2]))
    assert np.allclose(protocols.unitary(cirq_circuit), protocols.unitary(expected_cirq_circuit))
...
def test_from_braket_three_qubit_gates():
    braket_circuit = BKCircuit()
    instructions = [Instruction(braket_gates.CCNot(), target=[1, 2, 3]), Instruction(braket_gates.CSwap(), target=[1, 2, 3])]
    for instr in instructions:
        braket_circuit.add_instruction(instr)
    cirq_circuit = from_braket(braket_circuit)
...
def _rotation_of_pi_over_7(num_qubits):
    ...
    matrix[0:2, 0:2] = [[np.cos(np.pi / 7), np.sin(np.pi / 7)], [-np.sin(np.pi / 7), np.cos(np.pi / 7)]]
    return matrix
...
def test_from_braket_raises_on_unsupported_gates():
    for num_qubits in range(1, 5):
        braket_circuit = BKCircuit()
        instr = Instruction(braket_gates.Unitary(_rotation_of_pi_over_7(num_qubits)), target=list(range(num_qubits)))
        braket_circuit.add_instruction(instr)
...
@pytest.mark.parametrize('uncommon_gate', [ops.HPowGate(exponent=-1 / 14), cirq_ionq_ops.GPIGate(phi=1 / 14), cirq_ionq_ops.GPI2Gate(phi=1 / 14)])
def test_to_from_braket_uncommon_one_qubit_gates(uncommon_gate):
    ...
    test_circuit = from_braket(to_braket(cirq_circuit))
    testing.assert_allclose_up_to_global_phase(protocols.unitary(test_circuit), protocols.unitary(cirq_circuit), atol=1e-07)

@pytest.mark.parametrize('common_gate', [ops.CNOT, ops.CZ, ops.ISWAP, ops.XXPowGate(exponent=-0.2), ops.YYPowGate(exponent=0.3), ops.ZZPowGate(exponent=-0.1)])
...
@pytest.mark.parametrize('uncommon_gate', [ops.CNotPowGate(exponent=-1 / 17), ops.CZPowGate(exponent=2 / 7), cirq_ionq_ops.MSGate(phi0=1 / 7, phi1=2 / 7)])
...
@pytest.mark.parametrize('common_gate', [ops.TOFFOLI, ops.FREDKIN])
