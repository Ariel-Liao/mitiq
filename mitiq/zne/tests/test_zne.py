...
import cirq
import qiskit
from mitiq.zne import inference, scaling, execute_with_zne, mitigate_executor, zne_decorator
from mitiq.zne.inference import AdaExpFactory, LinearFactory, PolyFactory, RichardsonFactory
from mitiq.zne.scaling import fold_gates_from_left, fold_gates_from_right, fold_gates_at_random, insert_id_layers
from mitiq import QPROGRAM, SUPPORTED_PROGRAM_TYPES
from mitiq.benchmarks.randomized_benchmarking import generate_rb_circuits
from mitiq.interface.mitiq_qiskit import execute_with_shots_and_noise, initialized_depolarizing_noise
from mitiq.interface import convert_from_mitiq, accept_any_qprogram_as_input
from mitiq.interface.mitiq_cirq import sample_bitstrings, compute_density_matrix
...
def test_with_observable_batched_factory(executor):
    ...
    zne_value = execute_with_zne(circuit, executor=functools.partial(executor, noise_model=cirq.depolarize), observable=observable, factory=PolyFactory(scale_factors=[1, 3, 5], order=2))
    true_value = observable.expectation(circuit, functools.partial(compute_density_matrix, noise_level=(0,)))
...
def test_with_observable_adaptive_factory(executor):
    ...
    circuit = cirq.Circuit(cirq.H.on(cirq.LineQubit(0))) * 20
    noisy_value = observable.expectation(circuit, sample_bitstrings)
    zne_value = execute_with_zne(circuit, executor=functools.partial(executor, noise_model=cirq.amplitude_damp), observable=observable, factory=AdaExpFactory(steps=4, asymptote=0.5))
    true_value = observable.expectation(circuit, functools.partial(compute_density_matrix, noise_level=(0,)))
    assert abs(zne_value - true_value) <= abs(noisy_value - true_value)

@pytest.mark.parametrize('executor', (sample_bitstrings, compute_density_matrix))
def test_with_observable_two_qubits(executor):
    observable = Observable(PauliString(spec='XX', coeff=-1.21), PauliString(spec='ZZ', coeff=0.7))
    circuit = cirq.Circuit(cirq.H.on(cirq.LineQubit(0)), cirq.CNOT.on(*cirq.LineQubit.range(2)))
...
@pytest.mark.parametrize('fold_method', [fold_gates_from_left, fold_gates_from_right, fold_gates_at_random, insert_id_layers])
...
def test_execute_with_zne_no_noise(fold_method, factory, num_to_average):
    ...
    zne_value = execute_with_zne(circ, executor, num_to_average=num_to_average, scale_noise=fold_method, factory=factory([1.0, 2.0, 3.0]))
...
def test_averaging_improves_zne_value_with_fake_noise(factory, fold_method):
    ...
    for seed in range(5):
        ...
        zne_value_no_averaging = execute_with_zne(circ, noisy_executor, num_to_average=1, scale_noise=fold_gates_at_random, factory=factory([1.0, 2.0, 3.0]))
        zne_value_averaging = execute_with_zne(circ, noisy_executor, num_to_average=10, scale_noise=fold_method, factory=factory([1.0, 2.0, 3.0]))
...
def qiskit_executor(qp: QPROGRAM, shots: int=10000) -> float:
    expectation = execute_with_shots_and_noise(qp, shots=shots, obs=ONE_QUBIT_GS_PROJECTOR, noise_model=initialized_depolarizing_noise(BASE_NOISE), seed=1)
    return expectation

def get_counts(circuit: qiskit.QuantumCircuit):
    return qiskit.execute(circuit, qiskit.Aer.get_backend('aer_simulator'), shots=100).result().get_counts()

@pytest.mark.order(0)
def test_qiskit_execute_with_zne():
    true_zne_value = 1.0
    circuit = qiskit_measure(*generate_rb_circuits(n_qubits=1, num_cliffords=TEST_DEPTH, trials=1, return_type='qiskit'), 0)
...
def test_qiskit_mitigate_executor():
    ...
    assert [abs(true_zne_value - batched_zne_value) < abs(true_zne_value - base) for batched_zne_value in batched_zne_values]
...
def test_qiskit_run_factory_with_number_of_shots():
    ...
    shot_list = [10000, 30000]
    fac = inference.ExpFactory(scale_factors=scale_factors, shot_list=shot_list, asymptote=0.5)
    circuit = qiskit_measure(*generate_rb_circuits(n_qubits=1, num_cliffords=TEST_DEPTH, trials=1, return_type='qiskit'), 0)
    base = qiskit_executor(circuit)
    zne_value = fac.run(circuit, qiskit_executor, scale_noise=scaling.fold_gates_at_random).reduce()
    assert abs(true_zne_value - zne_value) < abs(true_zne_value - base)
    for i in range(len(fac._instack)):
        assert fac._instack[i] == {'scale_factor': scale_factors[i], 'shots': shot_list[i]}
...
def test_execute_with_zne_with_supported_circuits(circuit_type):
    ...
    cirq_circuit = cirq.Circuit(cirq.H.on_each(qreg), cirq.CNOT(*qreg), cirq.CNOT(*qreg), cirq.H.on_each(qreg))
