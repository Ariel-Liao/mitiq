...
import numpy as np
import pytest
from cirq import Circuit, GridQubit, LineQubit, ops, inverse, equal_up_to_global_phase, InsertStrategy, testing, ry, rz
from qiskit import ClassicalRegister, QuantumCircuit, QuantumRegister
from qiskit.quantum_info.operators import Operator
from pyquil import Program, gates
from pyquil.quilbase import Pragma
from mitiq.utils import _equal
from mitiq.interface import CircuitConversionError, convert_to_mitiq, convert_from_mitiq
from mitiq.zne.scaling.folding import UnfoldableCircuitError, _squash_moments, _default_weight, _fold_all, fold_all, fold_gates_from_left, fold_gates_from_right, fold_gates_at_random, fold_global, _create_weight_mask, _create_fold_mask, _apply_fold_mask
...
def test_fold_all(scale_factor, with_measurements):
    ...
    circ = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(qreg[0], qreg[1])], [ops.X.on(qreg[2])], [ops.TOFFOLI.on(*qreg)])
    if with_measurements:
        circ.append(ops.measure_each(*qreg))
    folded = fold_all(circ, scale_factor=scale_factor)
    correct = Circuit([ops.H.on_each(*qreg)] * scale_factor, [ops.CNOT.on(qreg[0], qreg[1])] * scale_factor, [ops.X.on(qreg[2])] * scale_factor, [ops.TOFFOLI.on(*qreg)] * scale_factor)
    if with_measurements:
...
def test_fold_all_exclude_with_gates():
    circuit = Circuit([ops.H(LineQubit(0))], [ops.CNOT(*LineQubit.range(2))], [ops.TOFFOLI(*LineQubit.range(3))])
    folded = fold_all(circuit, scale_factor=3.0, exclude={ops.H})
    correct = Circuit([ops.H(LineQubit(0))], [ops.CNOT(*LineQubit.range(2))] * 3, [ops.TOFFOLI(*LineQubit.range(3))] * 3)
    assert _equal(folded, correct, require_qubit_equality=True)
    folded = fold_all(circuit, scale_factor=3.0, exclude={ops.CNOT})
    correct = Circuit([ops.H(LineQubit(0))] * 3, [ops.CNOT(*LineQubit.range(2))], [ops.TOFFOLI(*LineQubit.range(3))] * 3)
    assert _equal(folded, correct, require_qubit_equality=True)
    folded = fold_all(circuit, scale_factor=5.0, exclude={ops.H, ops.TOFFOLI})
    correct = Circuit([ops.H(LineQubit(0))], [ops.CNOT(*LineQubit.range(2))] * 5, [ops.TOFFOLI(*LineQubit.range(3))])
    assert _equal(folded, correct, require_qubit_equality=True)
...
def test_fold_all_skip_moments(skip):
    ...
    for i, moment in enumerate(circuit):
        ...
        for _ in range(times_to_add):
            correct += moment
    assert _equal(_squash_moments(folded), _squash_moments(correct), require_qubit_equality=True)

@pytest.mark.order(0)
def test_folding_with_bad_scale_factor():
    for fold_function in (fold_all, fold_gates_at_random, fold_gates_from_left, fold_gates_from_right):
...
def test_fold_all_bad_exclude():
    with pytest.raises(ValueError, match='Do not know how to parse'):
        fold_all(Circuit(), scale_factor=1.0, exclude=frozenset(('not a gate name',)))
...
def test_fold_from_left_two_qubits():
    qreg = LineQubit.range(2)
    circ = Circuit([ops.H.on_each(*qreg), ops.CNOT.on(qreg[0], qreg[1]), ops.T.on(qreg[1])])
    folded = fold_gates_from_left(circ, scale_factor=2.5)
    correct = Circuit([ops.H.on_each(*qreg)] * 3, [ops.CNOT.on(*qreg)] * 3, [ops.T.on(qreg[1])])
    assert _equal(folded, _squash_moments(correct))
    folded = fold_gates_from_left(circ, scale_factor=3)
    correct = Circuit([ops.H.on_each(*qreg)] * 3, [ops.CNOT.on(*qreg)] * 3, [ops.T.on(qreg[1]), ops.T.on(qreg[1]) ** (-1), ops.T.on(qreg[1])])
...
def test_fold_from_left_three_qubits():
    ...
    qreg = LineQubit.range(3)
    circ = Circuit([ops.H.on_each(*qreg), ops.CNOT.on(qreg[0], qreg[1]), ops.T.on(qreg[2]), ops.TOFFOLI.on(*qreg)])
...
def test_fold_from_left_with_terminal_measurements_min_stretch():
    ...
    qreg = LineQubit.range(3)
    circ = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(qreg[0], qreg[1])], [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)], [ops.measure_each(*qreg)])
    folded = fold_gates_from_left(circ, scale_factor=1.0)
    correct = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(qreg[0], qreg[1])], [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)], [ops.measure_each(*qreg)])
    assert _equal(folded, correct)
...
def test_fold_from_left_with_terminal_measurements_max_stretch():
    ...
    correct = Circuit([ops.H.on_each(*qreg)] * 3, [ops.CNOT.on(qreg[0], qreg[1])] * 3, [ops.T.on(qreg[2]), ops.T.on(qreg[2]) ** (-1), ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)] * 3, [ops.measure_each(*qreg)])
    assert _equal(folded, correct)

@pytest.mark.parametrize('fold_method', [fold_gates_from_left, fold_gates_from_right, fold_gates_at_random, fold_global])
def test_fold_with_intermediate_measurements_raises_error(fold_method):
    ...
    with pytest.raises(UnfoldableCircuitError, match='Circuit contains intermediate measurements'):
...
def test_parametrized_circuit_folding_terminal_measurement(fold_method):
    ...
    folded_circ = fold_method(ansatz_circ, scale_factor=3.0)
    expected_circ = Circuit(ry(theta).on(q), ry(-theta).on(q), ry(theta).on(q), ops.measure(q))
    assert _equal(folded_circ, expected_circ)
...
def test_fold_from_right_basic():
    ...
    correct = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(*qreg)], [ops.T.on(qreg[1]), ops.T.on(qreg[1]) ** (-1), ops.T.on(qreg[1])])
    assert _equal(folded, correct)
    folded = fold_gates_from_right(circ, scale_factor=2.5)
    correct = Circuit([ops.H.on(qreg[0])], [ops.H.on(qreg[1])] * 3, [ops.CNOT.on(*qreg)] * 3, [ops.T.on(qreg[1]), ops.T.on(qreg[1]) ** (-1), ops.T.on(qreg[1])])
    assert _equal(folded, correct)
...
def test_fold_from_right_with_terminal_measurements_max_stretch():
    ...
    correct = Circuit([ops.H.on_each(*qreg)] * 3, [ops.CNOT.on(qreg[0], qreg[1])] * 3, [ops.T.on(qreg[2]), ops.T.on(qreg[2]) ** (-1), ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)] * 3, [ops.measure_each(*qreg)])
    assert _equal(folded, correct)
    original = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(qreg[0], qreg[1])], [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)], [ops.measure_each(*qreg)])
...
def test_local_folding_methods_match_on_odd_scale_factors():
    circuit = testing.random_circuit(qubits=3, n_moments=5, op_density=1.0, random_state=11)
    for s in (3, 7, 15):
        assert _equal(fold_gates_from_left(circuit, s), fold_gates_from_right(circuit, s), require_qubit_equality=True, require_measurement_equality=True)
    for s in (3, 7, 15):
        assert _equal(fold_gates_from_left(circuit, s), fold_gates_at_random(circuit, s), require_qubit_equality=True, require_measurement_equality=True)
...
def test_fold_gates_at_random_seed_one_qubit():
    ...
    correct = Circuit([ops.X.on(qubit)] * 3, [ops.Y.on(qubit)] * 3, [ops.Z.on(qubit)] * 3)
    assert _equal(folded, correct)
...
def test_fold_random_max_stretch():
    ...
    correct = Circuit([ops.H.on_each(*qreg)] * 3, [ops.CNOT.on(qreg[0], qreg[1])] * 3, [ops.T.on(qreg[2]), ops.T.on(qreg[2]) ** (-1), ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)] * 3)
    assert _equal(folded, correct)
...
def test_fold_random_no_repeats():
    ...
    """
    qreg = LineQubit.range(2)
    circ = Circuit([ops.H.on_each(qreg[0])], [ops.CNOT.on(*qreg)], [ops.X.on(qreg[1])], [ops.Y.on(qreg[0])], [ops.CZ.on(*qreg)])
...
def test_global_fold_stretch_factor_eight_terminal_measurements():
    ...
    correct = Circuit(circ, inverse(circ), circ, inverse(Circuit([ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)])), [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)], meas)
    assert _equal(folded, correct)
...
def test_global_fold_moment_structure_maintained_full_scale_factors():
    ...
    assert _equal(folded, correct)
    qreg = LineQubit.range(3)
    gate_list = [ops.CNOT.on(qreg[0], qreg[1]), [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)]]
...
def test_fold_from_left_with_qiskit_circuits():
    ...
    folded_circuit = fold_gates_from_left(qiskit_circuit, scale_factor=1.0, return_mitiq=True)
    qreg = LineQubit.range(3)
    correct_folded_circuit = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(qreg[0], qreg[1])], [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)], [ops.measure_each(*qreg)])
    assert isinstance(folded_circuit, Circuit)
    assert _equal(folded_circuit, correct_folded_circuit)
    qiskit_folded_circuit = fold_gates_from_left(qiskit_circuit, scale_factor=1.0, return_mitiq=False)
...
def test_fold_from_right_with_qiskit_circuits():
    ...
    qiskit_circuit.ccx(*qiskit_qreg)
    qiskit_circuit.measure(qiskit_qreg, qiskit_creg)
    folded_circuit = fold_gates_from_right(qiskit_circuit, scale_factor=1.0, return_mitiq=True)
    qreg = LineQubit.range(3)
    correct_folded_circuit = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(qreg[0], qreg[1])], [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)], [ops.measure_each(*qreg)])
    assert isinstance(folded_circuit, Circuit)
    assert _equal(folded_circuit, correct_folded_circuit)
...
def test_fold_at_random_with_qiskit_circuits():
    ...
    qiskit_circuit.ccx(*qiskit_qreg)
    qiskit_circuit.measure(qiskit_qreg, qiskit_creg)
    folded_circuit = fold_gates_at_random(qiskit_circuit, scale_factor=1.0, return_mitiq=True)
    qreg = LineQubit.range(3)
    correct_folded_circuit = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(qreg[0], qreg[1])], [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)], [ops.measure_each(*qreg)])
    assert isinstance(folded_circuit, Circuit)
    assert _equal(folded_circuit, correct_folded_circuit)
...
def test_fold_global_with_qiskit_circuits():
    ...
    qiskit_circuit.ccx(*qiskit_qreg)
    qiskit_circuit.measure(qiskit_qreg, qiskit_creg)
    folded_circuit = fold_global(qiskit_circuit, scale_factor=2.71828, fold_method=fold_gates_from_left, return_mitiq=True)
    assert isinstance(folded_circuit, Circuit)
    folded_qiskit_circuit = fold_global(qiskit_circuit, scale_factor=2.0, fold_method=fold_gates_from_left)
...
def test_fold_left_squash_moments():
    ...
    qreg = LineQubit.range(3)
    circ = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(qreg[0], qreg[1])], [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)], [ops.measure_each(*qreg)])
    folded_not_squashed = fold_gates_from_left(circ, scale_factor=3, squash_moments=False)
    folded_and_squashed = fold_gates_from_left(circ, scale_factor=3, squash_moments=True)
    correct = Circuit([ops.H.on_each(*qreg)] * 3, [ops.CNOT.on(qreg[0], qreg[1])] * 3, [ops.T.on(qreg[2]), ops.T.on(qreg[2]) ** (-1), ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)] * 3, [ops.measure_each(*qreg)])
...
@pytest.mark.parametrize('fold_method', [fold_gates_from_left, fold_gates_from_right, fold_gates_at_random])
def test_fold_and_squash_max_stretch(fold_method):
    ...
    for i in range(d):
        circuit.insert(0, ops.H.on(qreg[i % 2]), strategy=InsertStrategy.NEW)
    folded_not_squashed = fold_method(circuit, scale_factor=3.0, squash_moments=False)
    folded_and_squashed = fold_method(circuit, scale_factor=3.0, squash_moments=True)
    folded_with_squash_moments_not_specified = fold_method(circuit, scale_factor=3.0)
...
def test_fold_and_squash_random_circuits_random_stretches(fold_method):
    ...
    for trial in range(5):
        ...
        folded_not_squashed = fold_method(circuit, scale_factor=scale, squash_moments=False, seed=trial)
        folded_and_squashed = fold_method(circuit, scale_factor=scale, squash_moments=True, seed=trial)
...
def test_fold_local_with_single_qubit_gates_fidelity_one(fold_method, qiskit):
    ...
    if qiskit:
        circ = convert_from_mitiq(circ, 'qiskit')
    folded = fold_method(circ, scale_factor=3.0, fidelities={'single': 1.0, 'CNOT': 0.99, 'TOFFOLI': 0.95})
    correct = Circuit([ops.H.on_each(*qreg)], [ops.CNOT.on(qreg[0], qreg[1])] * 3, [ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)] * 3)
    if qiskit:
...
def test_all_gates_folded_at_max_scale_with_fidelities(fold_method, qiskit):
    ...
    if qiskit:
        circ = convert_from_mitiq(circ, 'qiskit')
    folded = fold_method(circ, scale_factor=3.0, fidelities={'H': rng.rand(), 'T': rng.rand(), 'CNOT': rng.rand(), 'TOFFOLI': rng.rand()})
    correct = Circuit([ops.H.on_each(*qreg)] * 3, [ops.CNOT.on(qreg[0], qreg[1])] * 3, [ops.T.on(qreg[2]), ops.T.on(qreg[2]) ** (-1), ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)] * 3)
    if qiskit:
        assert folded.qregs == circ.qregs
        assert folded.cregs == circ.cregs
...
def test_convert_from_mitiq_circuit_conversion_error(conversion_type):
    ...
    with pytest.raises(CircuitConversionError, match='Circuit could not be converted from'):
...
def test_convert_qiskit_to_mitiq_circuit_conversion_error():
    ...
    circ.unitary(gate, [0])
    with pytest.raises(CircuitConversionError, match='Circuit could not be converted to'):
...
def test_fold_fidelity_large_scale_factor_only_twoq_gates(fold_method, scale):
    ...
    folded = fold_method(circuit, scale_factor=scale, fidelities={'single': 1.0})
...
def test_create_weight_mask_with_fidelities():
    qreg = LineQubit.range(3)
    circ = Circuit(ops.H.on_each(*qreg), ops.CNOT.on(qreg[0], qreg[1]), ops.T.on(qreg[2]), ops.ISWAP.on(qreg[1], qreg[0]), ops.TOFFOLI.on(*qreg), ops.measure_each(*qreg))
    fidelities = {'H': 0.9, 'CNOT': 0.8, 'T': 0.7, 'TOFFOLI': 0.6, 'ISWAP': 0.5}
    ...
    fidelities = {'single': 1.0, 'double': 1.0, 'H': 0.1, 'CNOT': 0.2, 'TOFFOLI': 0.3}
    weight_mask = _create_weight_mask(circ, fidelities)
...
@pytest.mark.parametrize('weight_mask', ([0.1, 0.2, 0.3, 0.0], [0.3, 0.5, 0.7, 0.0], [1.0, 1.0, 1.0, 0.0]))
@pytest.mark.parametrize('scale_factor', (1, 3, 5, 7, 9, 11))
@pytest.mark.parametrize('method', ('at_random', 'from_left', 'from_right'))
def test_create_fold_mask_with_odd_scale_factors(weight_mask, scale_factor, method):
...
def test_create_fold_mask_with_real_scale_factors_from_left():
    fold_mask = _create_fold_mask(weight_mask=[0.1, 0.2, 0.3, 0.0], scale_factor=1.0, folding_method='from_left')
    assert fold_mask == [0, 0, 0, 0]
    fold_mask = _create_fold_mask(weight_mask=[0.1, 0.1, 0.1, 0.1], scale_factor=1.5, folding_method='from_left')
    assert fold_mask == [1, 0, 0, 0]
    fold_mask = _create_fold_mask(weight_mask=[1.0, 1.0, 1.0, 1.0], scale_factor=2, folding_method='from_left')
    assert fold_mask == [1.0, 1.0, 0.0, 0.0]
    fold_mask = _create_fold_mask(weight_mask=[1.0, 1.0, 1.0, 1.0], scale_factor=3.9, folding_method='from_left')
    assert fold_mask == [2, 2, 1, 1]

@pytest.mark.order(0)
def test_create_fold_mask_with_real_scale_factors_from_right():
    fold_mask = _create_fold_mask(weight_mask=[0.1, 0.2, 0.3, 0.0], scale_factor=1.0, folding_method='from_right')
    assert fold_mask == [0, 0, 0, 0]
    fold_mask = _create_fold_mask(weight_mask=[0.1, 0.1, 0.1, 0.1], scale_factor=1.5, folding_method='from_right')
    assert fold_mask == [0, 0, 0, 1]
    fold_mask = _create_fold_mask(weight_mask=[1.0, 1.0, 1.0, 1.0], scale_factor=2, folding_method='from_right')
    assert fold_mask == [0.0, 0.0, 1.0, 1.0]
    fold_mask = _create_fold_mask(weight_mask=[1.0, 1.0, 1.0, 1.0], scale_factor=3.9, folding_method='from_right')
    assert fold_mask == [1, 1, 2, 2]

@pytest.mark.order(0)
def test_create_fold_mask_with_real_scale_factors_at_random():
    fold_mask = _create_fold_mask(weight_mask=[0.1, 0.2, 0.3, 0.0], scale_factor=1.0, folding_method='at_random', seed=1)
    assert fold_mask == [0, 0, 0, 0]
    fold_mask = _create_fold_mask(weight_mask=[0.1, 0.1, 0.1, 0.1, 0.0], scale_factor=1.5, folding_method='at_random', seed=2)
    assert fold_mask == [0, 0, 1, 0, 0]
    fold_mask = _create_fold_mask(weight_mask=[1, 1, 1, 1], scale_factor=2, folding_method='at_random', seed=3)
    assert fold_mask == [0, 1, 0, 1]
    fold_mask = _create_fold_mask(weight_mask=[1, 1, 1, 1], scale_factor=3.9, folding_method='at_random', seed=7)
...
def test_apply_fold_mask():
    ...
    correct = Circuit([ops.H.on_each(*qreg)] * 3, [ops.CNOT.on(qreg[0], qreg[1])] * 3, [ops.T.on(qreg[2]), inverse(ops.T.on(qreg[2])), ops.T.on(qreg[2])], [ops.TOFFOLI.on(*qreg)] * 3, [ops.measure_each(*qreg)])
    assert _equal(folded, correct)
...
def test_apply_fold_mask_with_squash_moments_option():
    ...
    correct = Circuit([ops.T.on_each(*q), inverse(ops.T(q[0])), ops.T(q[0])]) + Circuit(ops.H(q[1]))
    assert _equal(folded, correct)
    folded = _apply_fold_mask(circ, [1, 1, 0], squash_moments=False)
    correct = Circuit([ops.T.on_each(*q), inverse(ops.T.on_each(*q)), ops.T.on_each(*q), ops.H(q[1])])
    assert _equal(folded, correct)
    folded = _apply_fold_mask(circ, [1, 0, 0], squash_moments=True)
    correct = Circuit([ops.T.on_each(*q), inverse(ops.T(q[0])), ops.T(q[0]), ops.H(q[1])])
    assert _equal(folded, correct)
