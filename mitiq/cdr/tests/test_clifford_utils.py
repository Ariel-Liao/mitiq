...
from mitiq.cdr.clifford_utils import is_clifford_angle, is_clifford, closest_clifford, random_clifford, angle_to_proximity, angle_to_proximities, probabilistic_angle_to_clifford, count_non_cliffords, _CLIFFORD_ANGLES

@pytest.mark.parametrize('circuit_type', SUPPORTED_PROGRAM_TYPES.keys())
def test_is_clifford_with_clifford(circuit_type):
    circuit = convert_from_mitiq(cirq.Circuit(cirq.Z.on(cirq.LineQubit(0))), circuit_type)
    assert is_clifford(circuit)

@pytest.mark.parametrize('circuit_type', SUPPORTED_PROGRAM_TYPES.keys())
def test_is_clifford_with_nonclifford(circuit_type):
    circuit = convert_from_mitiq(cirq.Circuit(cirq.T.on(cirq.LineQubit(0))), circuit_type)
    assert not is_clifford(circuit)

@pytest.mark.parametrize('circuit_type', SUPPORTED_PROGRAM_TYPES.keys())
def test_count_non_cliffords(circuit_type):
    a, b = cirq.LineQubit.range(2)
    circuit = Circuit(cirq.rz(0.0).on(a), cirq.rx(0.1 * np.pi).on(b), cirq.rx(0.5 * np.pi).on(b), cirq.rz(0.4 * np.pi).on(b), cirq.rz(0.5 * np.pi).on(b), cirq.CNOT.on(a, b))
...
def test_probabilistic_angles_to_clifford():
    for sigma in np.linspace(0.1, 2, 10):
        angles = probabilistic_angle_to_clifford(_CLIFFORD_ANGLES, sigma, np.random.RandomState(1))
