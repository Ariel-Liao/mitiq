...
from mitiq.cdr.clifford_training_data import _select, _map_to_near_clifford, _replace, generate_training_circuits
from mitiq.cdr.clifford_utils import is_clifford, count_non_cliffords
...
def test_map_to_near_clifford():
    ...
    new_ops = _map_to_near_clifford(ops, fraction_non_clifford=0.0, method_select='uniform', method_replace='uniform', random_state=np.random.RandomState(2))
...
def test_generate_training_circuits_bad_methods():
    with pytest.raises(ValueError):
        generate_training_circuits(Circuit(cirq.ops.rx(0.5).on(cirq.LineQubit(0))), num_training_circuits=1, fraction_non_clifford=0.0, method_select='unknown select method')
    with pytest.raises(ValueError):
        generate_training_circuits(Circuit(cirq.ops.rx(0.5).on(cirq.LineQubit(0))), num_training_circuits=1, fraction_non_clifford=0.0, method_replace='unknown replace method')

@pytest.mark.order(0)
def test_generate_training_circuits_with_clifford_circuit():
    circuit = Circuit(cirq.ops.rx(0.0).on(cirq.LineQubit(0)))
    assert generate_training_circuits(circuit, num_training_circuits=2, fraction_non_clifford=0.0) == [circuit, circuit]
...
def test_generate_training_circuits_mega(method_select, method_replace, kwargs):
    ...
    train_circuits = generate_training_circuits(circuit, num_training_circuits=num_train, fraction_non_clifford=0.1, random_state=np.random.RandomState(13), method_select=method_select, method_replace=method_replace, **kwargs)
    ...
    for train_circuit in train_circuits:
        ...
        assert count_non_cliffords(train_circuit) == int(round(fraction_non_clifford * count_non_cliffords(circuit)))
...
def test_select(method):
    ...
    indices = _select(non_clifford_ops, fraction_non_clifford, method=method, sigma=0.5, random_state=np.random.RandomState(1))
    assert all((isinstance(index, int) for index in indices))
    assert len(indices) == int(round((1.0 - fraction_non_clifford) * len(non_clifford_ops)))