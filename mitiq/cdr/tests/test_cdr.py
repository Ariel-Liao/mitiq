...
from mitiq.cdr import execute_with_cdr, linear_fit_function_no_intercept, linear_fit_function, mitigate_executor, cdr_decorator
...
@pytest.mark.parametrize('fit_function', [linear_fit_function, linear_fit_function_no_intercept])
@pytest.mark.parametrize('kwargs', [{}, {'method_select': 'gaussian', 'method_replace': 'gaussian', 'sigma_select': 0.5, 'sigma_replace': 0.5}])
@pytest.mark.parametrize('random_state', [1, 2, 3, 4, 5])
def test_execute_with_cdr(circuit_type, fit_function, kwargs, random_state):
    circuit = random_x_z_cnot_circuit(LineQubit.range(2), n_moments=5, random_state=random_state)
    ...
    cdr_value = execute_with_cdr(circuit, execute, obs, simulator=simulate, num_training_circuits=20, fraction_non_clifford=0.5, fit_function=fit_function, random_state=random_state, **kwargs)
...
def test_decorator_execute_with_cdr(circuit_type, fit_function, kwargs, random_state):
    ...
    @cdr_decorator(observable=obs, simulator=simulate, num_training_circuits=20, fraction_non_clifford=0.5, fit_function=fit_function, random_state=random_state, **kwargs)
...
def test_mitigated_execute_with_cdr(circuit_type, fit_function, kwargs, random_state):
    ...
    true_value = obs.expectation(circuit, simulate)
    noisy_value = obs.expectation(circuit, execute)
    cdr_executor = mitigate_executor(executor=execute, observable=obs, simulator=simulate, num_training_circuits=20, fraction_non_clifford=0.5, fit_function=fit_function, random_state=random_state, **kwargs)
    cdr_mitigated = cdr_executor(circuit)
    assert abs(cdr_mitigated - true_value) <= abs(noisy_value - true_value)
    cdr_batched_executor = mitigate_executor(executor=batched_execute, observable=obs, simulator=simulate, num_training_circuits=20, fraction_non_clifford=0.5, fit_function=fit_function, random_state=random_state, **kwargs)
    cdr_batched_mitigated_values = cdr_batched_executor([circuit] * 3)
    assert [abs(cdr_batched_mitigated - true_value) <= abs(noisy_value - true_value) for cdr_batched_mitigated in cdr_batched_mitigated_values]
...
def test_execute_with_variable_noise_cdr(circuit_type):
    ...
    true_value = obs.expectation(circuit, simulate)
    noisy_value = obs.expectation(circuit, execute)
    vncdr_value = execute_with_cdr(circuit, execute, obs, simulator=simulate, num_training_circuits=10, fraction_non_clifford=0.5, scale_factors=[1, 3], random_state=1)
...
def test_mitigate_executor_with_variable_noise_cdr(circuit_type):
    ...
    true_value = obs.expectation(circuit, simulate)
    noisy_value = obs.expectation(circuit, execute)
    vncdr_executor = mitigate_executor(executor=execute, observable=obs, simulator=simulate, num_training_circuits=10, fraction_non_clifford=0.5, scale_factors=[1, 3], random_state=1)
...
def test_no_num_fit_parameters_with_custom_fit_raises_error():
    with pytest.raises(ValueError, match='Must provide `num_fit_parameters`'):
        execute_with_cdr(random_x_z_cnot_circuit(LineQubit.range(2), n_moments=2, random_state=1), execute, observables=Observable(PauliString()), simulator=simulate, fit_function=lambda _: 1)

@pytest.mark.order(0)
def test_no_num_fit_parameters_mitigate_executor_raises_error():
    with pytest.raises(ValueError, match='Must provide `num_fit_parameters`'):
        mitigated_executor = mitigate_executor(executor=execute, observables=Observable(PauliString()), simulator=simulate, fit_function=lambda _: 1)
        mitigated = (mitigated_executor(random_x_z_cnot_circuit(LineQubit.range(2), n_moments=2, random_state=1)),)
        mitigated
