...
@pytest.mark.parametrize('qubit', (cirq.LineQubit(0), cirq.GridQubit(1, 2), cirq.NamedQubit('Qubit')))
def test_init_with_different_qubits(qubit):
    ideal_op = Circuit(cirq.H.on(qubit))
    real = np.zeros(shape=(4, 4))
    noisy_op = NoisyOperation(ideal_op, real)
    assert isinstance(noisy_op._circuit, cirq.Circuit)
    assert _equal(noisy_op.circuit, cirq.Circuit(ideal_op), require_qubit_equality=True)
...
@pytest.mark.parametrize('gate', (cirq.H, cirq.H(cirq.LineQubit(0)), qiskit.extensions.HGate, qiskit.extensions.CHGate, pyquil.gates.H))
...
def test_add_simple():
    ...
    correct = cirq.Circuit([cirq.X.on(cirq.NamedQubit('Q')), cirq.Y.on(cirq.NamedQubit('Q'))])
...
def test_add_noisy_operation_no_channel_matrix():
    ...
    noisy_op2 = NoisyOperation(cirq.Circuit([cirq.X.on(cirq.NamedQubit('Q'))]), channel_matrix=np.random.rand(4, 4))
...
def get_test_representation():
    ...
    decomp = OperationRepresentation(ideal, [noisy_xop, noisy_zop], [0.5, -0.5])
...
def test_representation_bad_type():
    ...
    with pytest.raises(TypeError, match='All elements of `noisy_operations`'):
        OperationRepresentation(ideal=ideal, noisy_operations=[0.1], coeffs=[0.1])
    with pytest.raises(TypeError, match='All elements of `coeffs` must'):
        OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop], coeffs=['x'])
...
def test_print_cirq_operation_representation():
    ...
    noisy_xop = NoisyOperation(circuit=xcirq, channel_matrix=np.zeros(shape=(4, 4)))
    noisy_zop = NoisyOperation(circuit=zcirq, channel_matrix=np.zeros(shape=(4, 4)))
    decomp = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop, noisy_zop], coeffs=[0.5, 0.5])
    expected = '0: ───H─── = 0.500*(0: ───X───)+0.500*(0: ───Z───)'
    assert str(decomp) == expected
    decomp = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop, noisy_zop], coeffs=[-0.5, 1.5])
    expected = '0: ───H─── = -0.500*(0: ───X───)+1.500*(0: ───Z───)'
    assert str(decomp) == expected
    decomp = OperationRepresentation(ideal, [], [])
    expected = '0: ───H─── = 0.000'
    assert str(decomp) == expected
    decomp = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop, noisy_zop], coeffs=[1.00001, 1e-05])
    expected = '0: ───H─── = 1.000*(0: ───X───)'
    assert str(decomp) == expected
    decomp = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop, noisy_zop], coeffs=[1e-05, 1.00001])
    expected = '0: ───H─── = 1.000*(0: ───Z───)'
    decomp = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop], coeffs=[1e-05])
    expected = '0: ───H─── = 0.000'
    assert str(decomp) == expected
...
def test_print_operation_representation_two_qubits():
    ...
    noisy_a = NoisyOperation(circuit=cirq.Circuit(cirq.H.on_each(qreg), cirq.CNOT(*qreg), cirq.H.on_each(qreg)))
    noisy_b = NoisyOperation(circuit=cirq.Circuit(cirq.Z.on_each(qreg), cirq.CNOT(*qreg), cirq.Z.on_each(qreg)))
    decomp = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_a, noisy_b], coeffs=[0.5, 0.5])
    expected = f"\n0: ───@───\n      │\n1: ───X─── ={' '}\n\n0.500\n0: ───H───@───H───\n          │\n1: ───H───X───H───\n\n+0.500\n0: ───Z───@───Z───\n          │\n1: ───Z───X───Z───"
    expected = expected[1:]
    assert str(decomp) == expected
...
def test_print_operation_representation_two_qubits_neg():
    qreg = cirq.LineQubit.range(2)
    ideal = cirq.Circuit(cirq.CNOT(*qreg))
    noisy_a = NoisyOperation(circuit=cirq.Circuit(cirq.H.on_each(qreg[0]), cirq.CNOT(*qreg), cirq.H.on_each(qreg[1])))
    noisy_b = NoisyOperation(circuit=cirq.Circuit(cirq.Z.on_each(*qreg)))
    decomp = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_a, noisy_b], coeffs=[-0.5, 1.5])
    expected = f"\n0: ───@───\n      │\n1: ───X─── ={' '}\n\n-0.500\n0: ───H───@───────\n          │\n1: ───────X───H───\n\n+1.500\n0: ───Z───\n\n1: ───Z───"
    expected = expected[1:]
    assert str(decomp) == expected
...
def test_equal_method_of_representations():
    ...
    noisy_xop_a = NoisyOperation(circuit=cirq.Circuit(cirq.X(q)), channel_matrix=np.zeros(shape=(4, 4)))
    noisy_zop_a = NoisyOperation(circuit=cirq.Circuit(cirq.Z(q)), channel_matrix=np.zeros(shape=(4, 4)))
    rep_a = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop_a, noisy_zop_a], coeffs=[0.5, 0.5])
    noisy_xop_b = NoisyOperation(circuit=cirq.Circuit(cirq.X(q)), channel_matrix=np.ones(shape=(4, 4)))
    noisy_zop_b = NoisyOperation(circuit=cirq.Circuit(cirq.Z(q)), channel_matrix=np.ones(shape=(4, 4)))
    rep_b = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop_b, noisy_zop_b], coeffs=[0.5, 0.5])
    assert rep_a == rep_b
    ideal_b = cirq.Circuit(cirq.X(q))
    rep_b = OperationRepresentation(ideal=ideal_b, noisy_operations=[noisy_xop_b, noisy_zop_b], coeffs=[0.5, 0.5])
    assert rep_a != rep_b
    q_b = qiskit.QuantumRegister(1)
    ideal_b = qiskit.QuantumCircuit(q_b)
    ideal_b.x(q_b)
    noisy_opx = NoisyOperation(ideal_b)
    rep_b = OperationRepresentation(ideal=ideal_b, noisy_operations=[noisy_opx, noisy_opx], coeffs=[0.5, 0.5])
    assert rep_a != rep_b
    rep_b = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop_b], coeffs=[0.5])
    assert rep_a != rep_b
    noisy_diff = NoisyOperation(circuit=cirq.Circuit(cirq.H(q)))
    rep_b = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop_b, noisy_diff], coeffs=[0.5, 0.5])
    assert rep_a != rep_b
    rep_b = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop_b, noisy_zop_b], coeffs=[0.7, 0.5])
    assert rep_a != rep_b
    rep_b = OperationRepresentation(ideal=ideal, noisy_operations=[noisy_xop_a, noisy_zop_a], coeffs=[0.5, 0.5], is_qubit_dependent=False)
    assert rep_a != rep_b

@pytest.mark.order(0)
def test_operation_representation_warnings():
    with pytest.warns(UserWarning, match='different from 1'):
        OperationRepresentation(ideal=xcirq, noisy_operations=[NoisyOperation(xcirq), NoisyOperation(zcirq)], coeffs=[0.5, 0.1])
...
def test_different_qubits_error():
    ...
    with pytest.raises(ValueError, match='must act on the same qubits'):
        OperationRepresentation(ideal=cirq.Circuit(cirq.X(cirq.NamedQubit('a'))), noisy_operations=[NoisyOperation(xcirq), NoisyOperation(zcirq)], coeffs=[0.5, 0.5])
...
def test_different_length_error():
    ...
    with pytest.raises(ValueError, match='must have equal length'):
        OperationRepresentation(ideal=cirq.Circuit(cirq.X(cirq.LineQubit(0))), noisy_operations=[NoisyOperation(xcirq), NoisyOperation(zcirq)], coeffs=[0.5, 0.5, 0.4])