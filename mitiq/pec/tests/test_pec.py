...
def get_pauli_and_cnot_representations(base_noise: float, qubits: Optional[List[cirq.Qid]]=None) -> List[OperationRepresentation]:
    if qubits is None:
    ...
    return represent_operations_in_circuit_with_local_depolarizing_noise(ideal_circuit=cirq.Circuit(ideal_operations), noise_level=base_noise)
BASE_NOISE = 0.02
...
def test_execute_with_pec_cirq_trivial_decomposition():
    ...
    rep = OperationRepresentation(circuit, [NoisyOperation(circuit)], [1.0])
    unmitigated = serial_executor(circuit)
    mitigated = execute_with_pec(circuit, serial_executor, representations=[rep], num_samples=10, random_state=1)
...
def test_execute_with_pec_cirq_noiseless_decomposition(circuit):
    ...
    mitigated = execute_with_pec(circuit, noiseless_serial_executor, representations=noiseless_pauli_representations, num_samples=10, random_state=1)
    assert np.isclose(unmitigated, mitigated)
...
def test_pyquil_noiseless_decomposition_multiqubit(nqubits):
    ...
    for q in range(nqubits):
        representation = OperationRepresentation(pyquil.Program(pyquil.gates.H(q)), [NoisyOperation(pyquil.Program(pyquil.gates.X(q))), NoisyOperation(pyquil.Program(pyquil.gates.Z(q)))], coeffs=[0.5, 0.5])
        representations.append(representation)
    exact = noiseless_serial_executor(circuit)
    pec_value = execute_with_pec(circuit, noiseless_serial_executor, representations=representations, num_samples=500, random_state=1)
...
def test_qiskit_noiseless_decomposition_multiqubit(nqubits):
    ...
    for q in qreg:
        ...
        zcircuit.z(q)
        representation = OperationRepresentation(opcircuit, [NoisyOperation(circuit=xcircuit), NoisyOperation(circuit=zcircuit)], [0.5, 0.5])
        representations.append(representation)
    exact = noiseless_serial_executor(circuit)
    pec_value = execute_with_pec(circuit, noiseless_serial_executor, representations=representations, num_samples=500, random_state=1)
    assert np.isclose(pec_value, exact, atol=0.1)
...
def test_execute_with_pec_with_observable():
    ...
    pec_value = execute_with_pec(circuit, executor, observable=obs, representations=pauli_representations, num_samples=100, force_run_all=False, random_state=101)
...
def test_execute_with_pec_partial_representations():
    ...
    pec_value = execute_with_pec(twoq_circ, executor=partial(mitiq_cirq.compute_density_matrix, noise_model=cirq.depolarize, noise_level=(BASE_NOISE,)), observable=Observable(PauliString('ZZ')), representations=reps, num_samples=100, force_run_all=False, random_state=101)
...
def test_execute_with_pec_with_different_samples(circuit, seed):
    ...
    for num_samples in (small_sample_number, large_sample_number):
        mitigated = execute_with_pec(circuit, serial_executor, representations=pauli_representations, num_samples=num_samples, force_run_all=True, random_state=seed)
...
def test_bad_precision_argument(bad_value: float):
    ...
    with pytest.raises(ValueError, match="The value of 'precision' should"):
        execute_with_pec(oneq_circ, serial_executor, representations=pauli_representations, precision=bad_value)
...
def test_large_sample_size_warning():
    ...
    with pytest.raises(Warning, match='The number of PEC samples is very large.'):
        execute_with_pec(oneq_circ, partial(fake_executor, random_state=np.random.RandomState(0)), representations=pauli_representations, num_samples=100001)
...
def test_pec_data_with_full_output():
    ...
    assert np.isclose(pec_data['pec_error'], np.std(pec_data['unbiased_estimators']) / np.sqrt(num_samples))
...
def test_mitigate_executor_qiskit():
    ...
    rep = OperationRepresentation(circuit, [NoisyOperation(circuit)], [1.0])
    unmitigated = serial_executor(circuit)
    mitigated_executor = mitigate_executor(serial_executor, representations=[rep], num_samples=10, random_state=1)
...
def test_executed_circuits_have_the_expected_type(circuit_type):
    ...
    def type_detecting_executor(circuit: QPROGRAM):
        ...
        return 0.0
    mitigated = execute_with_pec(circuit, executor=type_detecting_executor, representations=pauli_representations, num_samples=1)
