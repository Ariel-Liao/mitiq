import numpy as np
import pytest
from cirq import CCNOT, CNOT, CZ, ISWAP, X, Y, Z, H, SWAP, Gate, LineQubit, Circuit, DepolarizingChannel, MeasurementGate
from mitiq.pec.representations import represent_operation_with_global_depolarizing_noise, represent_operation_with_local_depolarizing_noise, represent_operations_in_circuit_with_global_depolarizing_noise, represent_operations_in_circuit_with_local_depolarizing_noise
from mitiq.pec.representations.depolarizing import global_depolarizing_kraus, local_depolarizing_kraus
...
def test_single_qubit_representation_norm(gate: Gate, noise: float):
    ...
    norm = represent_operation_with_global_depolarizing_noise(Circuit(gate(q)), noise).norm
...
def test_two_qubit_representation_norm(gate: Gate, noise: float):
    ...
    norm = represent_operation_with_global_depolarizing_noise(Circuit(gate(*qreg)), noise).norm
    assert np.isclose(optimal_norm, norm)
...
def test_represent_operations_in_circuit_global(circuit_type: str):
    ...
    reps = represent_operations_in_circuit_with_global_depolarizing_noise(ideal_circuit=circ, noise_level=0.1)
...
def test_represent_operations_in_circuit_local(circuit_type: str):
    ...
    circ_mitiq = Circuit([CNOT(*qreg), H(qreg[0]), Y(qreg[1]), CNOT(*qreg)])
    circ = convert_from_mitiq(circ_mitiq, circuit_type)
    reps = represent_operations_in_circuit_with_local_depolarizing_noise(ideal_circuit=circ, noise_level=0.1)
    for op in convert_to_mitiq(circ)[0].all_operations():
        found = False
        for rep in reps:
            if _equal(rep.ideal, Circuit(op), require_qubit_equality=True):
...
@pytest.mark.parametrize('rep_function', [represent_operations_in_circuit_with_local_depolarizing_noise, represent_operations_in_circuit_with_global_depolarizing_noise])
@pytest.mark.parametrize('circuit_type', ['cirq', 'qiskit', 'pyquil'])
...
@pytest.mark.parametrize('kraus_function', [global_depolarizing_kraus, local_depolarizing_kraus])
def test_depolarizing_kraus(kraus_function):
    expected = [[[0.5, 0.0], [0.0, 0.5]], [[0.0 + 0j, 0.5 + 0j], [0.5 + 0j, 0.0 + 0j]], [[0.0 + 0j, 0.0 - 0.5j], [0.0 + 0.5j, 0.0 + 0j]], [[0.5 + 0j, 0.0 + 0j], [0.0 + 0j, -0.5 + 0j]]]
