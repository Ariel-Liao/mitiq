import numpy as np
import pytest
from cirq import CCNOT, CNOT, CZ, ISWAP, I, X, Y, Z, H, SWAP, Gate, LineQubit, Circuit, ops, unitary
from mitiq.pec.representations.biased_noise import represent_operation_with_local_biased_noise
...
def single_qubit_biased_noise_overhead(epsilon: float, eta: float) -> float:
    ...
    c = epsilon / (3 * (eta + 1))
    eta1 = (a ** 2 + a * b - 2 * c ** 2) / (a ** 3 + a ** 2 * b - a * b ** 2 - 4 * a * c ** 2 - b ** 3 + 4 * b * c ** 2)
    eta2 = -c / (a ** 2 + 2 * a * b + b ** 2 - 4 * c ** 2)
    eta3 = (-a * b - b ** 2 + 2 * c ** 2) / (a ** 3 + a ** 2 * b - a * b ** 2 - 4 * a * c ** 2 - b ** 3 + 4 * b * c ** 2)
...
def two_qubit_biased_noise_overhead(epsilon: float, eta: float) -> float:
    ...
    b = epsilon * (3 * eta + 1) / (3 * (eta + 1))
    c = epsilon / (3 * (eta + 1))
    alpha_sq = (a ** 2 + a * b - 2 * c ** 2) ** 2 / (a ** 3 + a ** 2 * b - a * b ** 2 - 4 * a * c ** 2 - b ** 3 + 4 * b * c ** 2) ** 2
    alpha_beta = (a ** 2 + a * b - 2 * c ** 2) * (-a * b - b ** 2 + 2 * c ** 2) / (a ** 3 + a ** 2 * b - a * b ** 2 - 4 * a * c ** 2 - b ** 3 + 4 * b * c ** 2) ** 2
    alpha_gamma = -c * (a ** 2 + a * b - 2 * c ** 2) / ((a ** 2 + 2 * a * b + b ** 2 - 4 * c ** 2) * (a ** 3 + a ** 2 * b - a * b ** 2 - 4 * a * c ** 2 - b ** 3 + 4 * b * c ** 2))
    beta_sq = (-a * b - b ** 2 + 2 * c ** 2) ** 2 / (a ** 3 + a ** 2 * b - a * b ** 2 - 4 * a * c ** 2 - b ** 3 + 4 * b * c ** 2) ** 2
    beta_gamma = -c * (-a * b - b ** 2 + 2 * c ** 2) / ((a ** 2 + 2 * a * b + b ** 2 - 4 * c ** 2) * (a ** 3 + a ** 2 * b - a * b ** 2 - 4 * a * c ** 2 - b ** 3 + 4 * b * c ** 2))
    gamma_sq = c ** 2 / (a ** 2 + 2 * a * b + b ** 2 - 4 * c ** 2) ** 2
    overhead = alpha_sq + 4 * abs(alpha_gamma) + 2 * abs(alpha_beta) + 4 * gamma_sq + 4 * abs(beta_gamma) + beta_sq
    return overhead
...
def test_single_qubit_representation_norm(gate: Gate, epsilon: float, eta: float):
    ...
    norm = represent_operation_with_local_biased_noise(Circuit(gate(q)), epsilon, eta).norm
...
def test_two_qubit_representation_norm(gate: Gate, epsilon: float, eta: float):
    ...
    norm = represent_operation_with_local_biased_noise(Circuit(gate(*qreg)), epsilon, eta).norm
    assert np.isclose(optimal_norm, norm)
...
def test_biased_noise_representation_with_choi(gate: Gate, epsilon: float, eta: float):
    ...
    op_rep = represent_operation_with_local_biased_noise(Circuit(gate.on(*qreg)), epsilon, eta)
    choi_components = []
    a = 1 - epsilon
    b = epsilon * (3 * eta + 1) / (3 * (eta + 1))
    c = epsilon / (3 * (eta + 1))
