...
import numpy as np
import pytest
from cirq import CXPowGate, DepolarizingChannel, MixedUnitaryChannel, Rx, Rz, I, X, Y, Z, LineQubit, Circuit, ops, unitary
import qiskit
...
from mitiq.pec.representations.learning import depolarizing_noise_loss_function, biased_noise_loss_function, learn_depolarizing_noise_parameter, learn_biased_noise_parameters, _parse_learning_kwargs
...
training_circuits = generate_training_circuits(circuit=circuit, num_training_circuits=3, fraction_non_clifford=0, method_select='uniform', method_replace='closest')
...
@pytest.mark.parametrize('operations', [[Circuit(CNOT_ops[0][1])], [Circuit(Rx_ops[0][1])]])
def test_depolarizing_noise_loss_function(epsilon, operations):
    ...
    loss = depolarizing_noise_loss_function(epsilon=np.array([epsilon]), operations_to_mitigate=operations, training_circuits=training_circuits, ideal_values=ideal_values, noisy_executor=noisy_executor, pec_kwargs=pec_kwargs, observable=observable)
...
def test_biased_noise_loss_function(epsilon, eta, operations):
    ...
    noisy_executor = Executor(noisy_execute)
    noisy_values = np.array(noisy_executor.evaluate(training_circuits, observable))
    loss = biased_noise_loss_function(params=[epsilon, eta], operations_to_mitigate=operations, training_circuits=training_circuits, ideal_values=ideal_values, noisy_executor=noisy_executor, pec_kwargs=pec_kwargs, observable=observable)
    assert loss <= np.mean((noisy_values - ideal_values) ** 2)

@pytest.mark.parametrize('operations', [[Circuit(CNOT_ops[0][1])], [Circuit(Rz_ops[0][1])]])
def test_biased_noise_loss_compare_ideal(operations):
    ...
    def noisy_execute(circ: Circuit) -> np.ndarray:
        ...
        return ideal_execute(noisy_circ)
    noisy_executor = Executor(noisy_execute)
    loss = biased_noise_loss_function(params=[0, 0], operations_to_mitigate=operations, training_circuits=training_circuits, ideal_values=ideal_values, noisy_executor=noisy_executor, pec_kwargs=pec_kwargs, observable=observable)
    assert np.isclose(loss, 0)

@pytest.mark.parametrize('operations', [[to_qiskit(Circuit(CNOT_ops[0][1]))], [to_qiskit(Circuit(Rx_ops[0][1]))], [to_qiskit(Circuit(Rz_ops[0][1]))]])
def test_biased_noise_loss_function_qiskit(operations):
    ...
    qiskit_training_circuits = generate_training_circuits(circuit=qiskit_circuit, num_training_circuits=3, fraction_non_clifford=0.2, method_select='uniform', method_replace='closest', random_state=rng)
    ...
    loss = biased_noise_loss_function(params=[epsilon, 0], operations_to_mitigate=operations, training_circuits=qiskit_training_circuits, ideal_values=ideal_values, noisy_executor=noisy_executor_qiskit, pec_kwargs=pec_kwargs)
    assert loss <= np.mean((noisy_values - ideal_values) ** 2)
...
def test_learn_biased_noise_parameters(epsilon, eta):
    ...
    for tc in range(0, num_training_circuits):
        pec_data[:, :, tc] = np.loadtxt(os.path.join('./mitiq/pec/representations/tests/learning_pec_data', f'learning_pec_data_eps_{eps_string}eta_{eta}tc_{tc}.txt'))
    [success, epsilon_opt, eta_opt] = learn_biased_noise_parameters(operations_to_learn=operations_to_learn, circuit=circuit, ideal_executor=ideal_executor, noisy_executor=noisy_executor, num_training_circuits=num_training_circuits, fraction_non_clifford=0.2, training_random_state=np.random.RandomState(1), epsilon0=epsilon0, eta0=eta0, observable=observable, learning_kwargs={'pec_data': pec_data})
    assert success
