...
from mitiq.interface.mitiq_cirq.cirq_utils import sample_bitstrings, compute_density_matrix
...
def test_observable_partition_can_be_measured_with():
    ...
    obs = Observable(*[PauliString(spec=rng.choice(('I', 'X', 'Y', 'Z'), size=n, replace=True, p=(0.7, 0.1, 0.1, 0.1))) for _ in range(nterms)])
...
def test_observable_measure_in_needs_one_circuit_z():
    ...
    assert _equal(measures_obs_circuits[0], expected, require_qubit_equality=True, require_measurement_equality=True)
...
def test_observable_expectation_two_circuits(n, executor):
    ...
    circuit = cirq.Circuit(cirq.H.on_each(qubits))
    expectation = obs.expectation(circuit, executor)
    assert np.isclose(expectation, -2.0, atol=0.1)
...
def test_observable_expectation_supported_qubits(executor):
    ...
    assert np.isclose(obs.expectation(circuit, executor), 0.0, atol=0.05)