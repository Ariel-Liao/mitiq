...
def test_pauli_init_with_support():
    ...
    assert pauli._pauli == cirq.PauliString(cirq.X(cirq.LineQubit(support[0])), cirq.Z(cirq.LineQubit(support[1])))
...
def test_pauli_matrix_include_qubits():
    ...
    assert np.allclose(pauli.matrix(qubit_indices_to_include=[0, 1, 2]), np.kron(np.kron(xmat, imat), imat))
...
def test_pauli_measure_in_circuit(support, circuit_type):
    ...
    qreg = [cirq.NamedQubit(name) for name in names]
    expected = cirq.Circuit(base_circuit.all_operations(), xrotation.on(qreg[support[0]]), yrotation.on(qreg[support[1]]), cirq.measure(*[qreg[s] for s in support]))
    if circuit_type == 'cirq':
...
def test_expectation_from_measurements_identity(seed, nqubits):
    ...
    assert np.isclose(pauli._expectation_from_measurements(measurements), coeff)

@pytest.mark.order(0)
def test_expectation_from_measurements_two_qubits():
    measurements = MeasurementResult([[0, 1] * 1000])
...
def test_pstringcollection():
    ...
    assert pauli_collection.elements_by_weight == {1: Counter((x, iz)), 2: Counter((xz,)), 4: Counter((xzixx,))}
...
def test_pstringcollection_expectation_from_measurements():
    ...
    pset = PauliStringCollection(PauliString(spec='ZI', coeff=-2.0), PauliString(spec='IZ', coeff=5.0))
