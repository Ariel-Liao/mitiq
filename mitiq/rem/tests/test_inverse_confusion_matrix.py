...
from mitiq.rem.inverse_confusion_matrix import bitstrings_to_probability_vector, generate_inverse_confusion_matrix, generate_tensored_inverse_confusion_matrix, mitigate_measurements, sample_probability_vector, closest_positive_distribution
...
def test_probability_vector_roundtrip():
    for _ in range(10):
        pv = np.random.rand(4)
        pv /= np.sum(pv)
        assert isclose(np.linalg.norm(pv - bitstrings_to_probability_vector(sample_probability_vector(pv, 1000))), 0, abs_tol=0.1)
...
def test_generate_inverse_confusion_matrix():
    ...
    assert (generate_inverse_confusion_matrix(num_qubits, p0=1, p1=1) == np.flipud(identity)).all()

@pytest.mark.parametrize('num_qubits, confusion_matrices, expected', [(2, [np.identity(2), np.identity(2)], np.identity(4)), (2, [np.identity(4)], np.identity(4)), (3, [np.identity(4), np.identity(2)], np.identity(8)), (2, [np.flipud(np.identity(2)), np.flipud(np.identity(2))], np.flipud(np.identity(4))), (3, [np.flipud(np.identity(4)), np.flipud(np.identity(2))], np.flipud(np.identity(8))), (3, [np.identity(2), np.identity(2)], ValueError), (3, [np.flipud(np.identity(2)), np.identity(2), 0.5 * np.ones((2, 2))], np.linalg.pinv(reduce(np.kron, [np.flipud(np.identity(2)), np.identity(2), 0.5 * np.ones((2, 2))])))])
def test_generate_tensored_inverse_confusion_matrix(num_qubits, confusion_matrices, expected):
    if expected is ValueError:
        with pytest.raises(ValueError):
            generate_tensored_inverse_confusion_matrix(num_qubits, confusion_matrices)
    else:
        assert np.isclose(generate_tensored_inverse_confusion_matrix(num_qubits, confusion_matrices), expected).all()
